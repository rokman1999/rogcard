      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
        <div className="bg-white/[0.02] backdrop-blur-2xl border border-white/10 p-8 relative flex flex-col transition-all hover:border-white/30 hover:bg-white/[0.05]">
          ...
          <div className="flex justify-between items-center mt-auto border-t border-white/10 pt-4">
            ...
            <button onClick={wrapClick(() => handleBuyItem('boost', 50000, '확률 부스트'))} disabled={isProcessing} className="bg-white/10 text-white font-mono text-xs tracking-widest px-4 py-2 hover:bg-white hover:text-black">50,000 G</button>
          </div>
        </div>
        <div className="bg-white/[0.02] backdrop-blur-2xl border border-white/10 p-8 relative flex flex-col transition-all hover:border-white/30 hover:bg-white/[0.05]">
          <HUDCorner />
          ...
          <div className="flex justify-between items-center mt-auto border-t border-white/10 pt-4">
            ...
            <button onClick={wrapClick(() => handleBuyItem('protect', 150000, '하락/파괴 보호권'))} disabled={isProcessing} className="bg-white/10 text-white font-mono text-xs tracking-widest px-4 py-2 hover:bg-white hover:text-black">150,000 G</button>
          </div>
        </div>
        <div className="bg-white/[0.02] backdrop-blur-2xl border border-white/10 p-8 relative flex flex-col transition-all hover:border-white/30 hover:bg-white/[0.05]">
          <HUDCorner />
          ...
          <div className="flex justify-between items-center mt-auto border-t border-white/10 pt-4">
            ...
            <button onClick={wrapClick(() => handleBuyItem('slot', 50000, '카드 슬롯 확장'))} disabled={isProcessing || (userData?.maxSlots >= 10)} className="bg-white/10 text-white font-mono text-xs tracking-widest px-4 py-2 hover:bg-white hover:text-black disabled:opacity-30">50,000 G</button>
          </div>
        </div>
      </div>
        ...
        {FRAMES_DATA.map(f => (
          <div key={f.id} className="bg-white/[0.02] border border-white/10 p-5 flex items-center justify-between transition-colors hover:bg-white/[0.05]">
            ...
            {userData?.frames?.includes(f.id)
              ...
              : <button onClick={wrapClick(() => handleBuyItem(f.id, f.price, f.name))} className="px-4 py-2 bg-white/10 hover:bg-white hover:text-black font-mono text-[10px] whitespace-nowrap">{formatMoney(f.price)} G</button>
            }
          </div>
        ))}
      </div>
